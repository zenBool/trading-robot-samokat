...
from trading.db import db_helper_binance as db
from trading.enums.enums import ValuesMA
...
def percentile(symbol: str, timeframe: str, _engine: AsyncEngine) -> pd.DataFrame:
...
class MovingAverage(Indicator):
    ...
    def val(self, idx: int) -> ValuesMA:
        """
        idx указывает на искомый бар, которые сортированы в обратном хронологическом порядке.
    ...
    def _set_slope(self):
        ...
        df = percentile(self.symbol.name, self.timeframe.name, db.engine)
